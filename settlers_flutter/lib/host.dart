        ...
        children: <Widget>[
          ...
          BlocProvider(
            create: (BuildContext context) => GameBloc(
                "num_players", globals.numPlayers,
...
class HostMenu extends StatefulWidget {
  @override
  _HostMenuState createState() {
    return _HostMenuState();
  }
}

class _HostMenuState extends State<HostMenu> {
  void _showLobby() {
    Navigator.of(context).pushNamed('/host/lobby');
  }
...
class HostLobby extends StatefulWidget {
  @override
  _HostLobby createState() {
    return _HostLobby();
  }
}

class _HostLobby extends State<HostLobby> {
  bool init = false;
  LobbyManager manager = LobbyManager();

  void _showOrderScreen() {
    Navigator.of(context).pushNamed('/host/order');
  }
              ...
              child: BlocBuilder<GameBloc, GameState>(
                builder: (context, state) {
                  ...
                  if (state is GameLoaded) {
                    init = false;
                    globals.playerOrder =
                        state.gamedata.gameData['player_order'];
                    return Container();
                  }
                  return Center(
...
class LobbyManager {
  Stream<List<String>> get lobbyNow async* {
    final url = 'http://christiandahnke.ddns.net/gamedata/';
    Map<String, String> headers = new Map();
    headers['Accept'] = 'application/json';
    headers['Content-type'] = 'application/json';

    while (!globals.lobbyFull) {
      final response = await http.Client().put(url,
          body: jsonEncode({'game': globals.gameCode}), headers: headers);
      final json = jsonDecode(response.body);
      List players2 = json['player_order'];
      List<String> players = List<String>.from(players2);
      yield players;
      if (players.length >= globals.numPlayers) globals.lobbyFull = true;
      await Future.delayed(Duration(seconds: 2));
    }
  }
}

class HostOrder extends StatefulWidget {
  @override
  _HostOrder createState() {
    return _HostOrder();
  }
}

class _HostOrder extends State<HostOrder> {
  void _showColorScreen() {
    Navigator.of(context).pushNamed('/hostgame');
  }
        ...
        body: Stack(children: <Widget>[
          ...
          BlocProvider(
            create: (BuildContext context) =>
                GameBloc("%player_order%", "", repository: repository),
            child: BlocBuilder<GameBloc, GameState>(
              builder: (context, state) {
                if (state is GameEmpty) {
                  BlocProvider.of<GameBloc>(context).add(PutGame());
                ...
                if (state is GameLoaded) {
                  globals.playerOrder = state.gamedata.gameData['player_order'];
                  globals.playerNum =
                      globals.playerOrder.indexOf(globals.tempName);
                  print(globals.playerNum);
          ...
          BlocProvider(
            create: (BuildContext context) =>
                GameBloc("next_screen", '1', repository: repository),
            child: BlocBuilder<GameBloc, GameState>(
              builder: (context, state) {
                if (state is GameEmpty) {
                  BlocProvider.of<GameBloc>(context).add(PutGame());
...
//stores the tile center coordinates on a 1000x1000 grid
  void findCenters() {
    double right = (sqrt(3) * 80) / 2;
    double down = 1.5 * 80;
    double xi = 500 - right * 4; // hexagon x
    double yi = 500 - down * 2; // hexagon y
    double x = xi;
    double y = yi;
  ...
  void initVerts() {
    ...
    for (int i = 0; i < globals.tiles.length; i++) {
      ...
      globals.initVertices.add([hexagon[0] + x, hexagon[1] + y]);
      for (int l = 2; l < hexagon.length - 1; l += 2) {
        globals.initVertices.add([hexagon[l] + x, hexagon[l + 1] + y]);
      }
    }
  }

  //removes overlapping globals.ve`rtices
  void removeDuplicates() {
    for (int i = 0; i < globals.initVertices.length; i++) {
      int count = 0;
      for (int j = i + 1; j < globals.initVertices.length; j++) {
        if ((globals.initVertices[j][0]).round() ==
                (globals.initVertices[i][0]).round() &&
            (globals.initVertices[j][1]).round() ==
                (globals.initVertices[i][1]).round()) {
          for (int k = 0; k < 12; k++) {
            for (int l = 0; l < 5; l++) {
          ...
          globals.initVertices.removeRange(j, j + 1);
          count++;
          j -= 1;
        }
      }
  ...
  //constructs an undirected graph of all globals.initVertices and edges
  void constructGraph() {
    int numVerts = globals.initVertices.length;
    globals.vertGraph = new Graph(numVerts);
    globals.roadGraph = new Graph(numVerts);
  ...
  //checks the possible adjacent globals.initVertices and returns their indeces
  List getAdj(i) {
    List adj = [];
    double xi = globals.initVertices[i][0];
    double yi = globals.initVertices[i][1];
    double x = xi;
    double y = yi - 80;
    double up = -80 / 2;
    double right = (sqrt(3) * 80) / 2;
    for (int k = 0; k < 6; k++) {
      ...
      for (int j = 0; j < globals.initVertices.length; j++) {
        if ((globals.initVertices[j][0]).round() == (x).round() &&
            (globals.initVertices[j][1]).round() == (y).round()) {
          adj.add(j);
        }
      }
    }
  ...
  void vertexXs() {
    for (int i = 0; i < globals.initVertices.length; i++) {
      if (!globals.initvertexXs.contains(globals.initVertices[i][0].round()))
        globals.initvertexXs.add(globals.initVertices[i][0].round());
    }
    globals.initvertexXs.sort();
  }
  ...
  Widget build(BuildContext context) {
    if (!init) {
      ...
      globals.h = screenSize.height;
      globals.refscale = (globals.w < globals.h) ? globals.w : globals.h;
      globals.scalar = 1 + ((globals.refscale - 1000) / 1000);
      //debugging game
      globals.gameCode = '4qsc';
      globals.playerNum = 0;
      globals.numPlayers = 2;
      globals.playerOrder = ['chimk', 'coom'];
      globals.players = {
        "coom": {
          "color": "",
          "hand": [10, 11, 20, 5, 20],
      ...
      globals.players['chimk']['color'] = 2;
      globals.players['coom']['color'] = 4;
      globals.currPlayer = 0;
      globals.storedSettlements = [];
      globals.storedRoads = [];
      globals.storedCities = [];
      globals.putData = 'start';
      // globals.placeP = '';
      ...
      return Material(
        ...
        child:
            ...
            BlocProvider(
          create: (BuildContext context) =>
              GameBloc("tiles", globals.tiles, repository: repository),
          child: BlocBuilder<GameBloc, GameState>(
            builder: (context, state) {
              if (state is GameEmpty) {
                BlocProvider.of<GameBloc>(context).add(PutGame());
              ...
              if (state is GameLoaded) {
                ...
                return BlocProvider(
                  create: (BuildContext context) => GameBloc(
                      "tile_centers", globals.initCenters,
                      repository: repository),
                  child: BlocBuilder<GameBloc, GameState>(
                    builder: (context, state) {
                      if (state is GameEmpty) {
                                                              ...
                                                              if (state
                                                                ...
                                                                for (int i = 0;
                                                                  ...
                                                                  globals
                                                                      .vertices
                                                                      .add([]);
                                                                  globals.vertices[i].add(globals
                                                                              .w /
                                                                          2 +
                                                                      (globals.initVertices[i][0] -
                                                                              500) *
                                                                          globals
                                                                              .scalar);
                                                                  globals.vertices[i].add(globals
                                                                ...
                                                                for (int i = 0;
                                                                  ...
                                                                  globals
                                                                      .tileCenters
                                                                      .add([]);
                                                                  globals.tileCenters[i].add(globals
                                                                              .w /
                                                                          2 +
                                                                      (globals.initCenters[i][0] -
                                                                              500) *
                                                                          globals
                                                                              .scalar);
                                                                  globals.tileCenters[i].add(globals
                                                                ...
                                                                for (int i = 0;
                                                                    i <
                                                                        globals
                                                                            ...
                                                                            .length;
                                                                    i++) {
                                                                  globals.vertexXs.add(globals
                                                                              .h /
                                                                          2 +
                                                                      (globals.initvertexXs[i] -
                                                                              500) *
                                                                          globals
                                                                              .scalar);
                                                                }
    ...
    return Material(
        type: MaterialType.transparency,
        child: StreamBuilder(
            stream: manager.gameNow,
            builder: (context, snapshot) {
              globals.gameData = snapshot.data;
              if (snapshot.hasData) {
                if (globals.currPlayer != globals.gameData["curr_player"] &&
                    (globals.rolled || globals.start)) {
                  globals.currPlayer = globals.gameData["curr_player"];
                  globals.players = globals.gameData['players'];
                  globals.rolled = false;
                }
                if (globals.placeP == '' || globals.placeP == 'h') {
                  if (globals.storedSettlements.length <
                      globals.gameData["stored_settlements"].length)
                    globals.storedSettlements =
                        globals.gameData["stored_settlements"];
                  if (globals.storedRoads.length <
                      globals.gameData["stored_roads"].length)
                    globals.storedRoads = globals.gameData["stored_roads"];
                  if (globals.storedCities.length <
                      globals.gameData["stored_cities"].length)
                    globals.storedCities = globals.gameData["stored_cities"];
                  if (globals.start &&
                      globals.storedRoads.length == globals.numPlayers * 2)
                    globals.start = false;
                }
                if (globals.currPlayer != globals.playerNum &&
                    globals.diceNum != globals.gameData["dice_num"]) {
                  if (globals.gameData["dice_num"] != 0) globals.rolled = true;
                  globals.diceNum = globals.gameData["dice_num"];
                }
                if (globals.currPlayer != globals.playerNum &&
                    globals.robberLoc != globals.gameData["robber_loc"]) {
                  globals.robberLoc = globals.gameData["robber_loc"];
                }
                if (globals.gameData['largest_army'] > globals.largestArmy)
                  globals.largestArmy = globals.gameData['largest_army'];
                if (globals.gameData['monopoly'] != -1) {}
...
class GameManager {
  Stream get gameNow async* {
    ...
    while (true) {
      var response;
      if (globals.putData == 'start') {
        globals.putData = '';
        response = await http.Client().put(url,
            body: jsonEncode({
              'curr_player': globals.currPlayer,
              'stored_settlements': globals.storedSettlements,
              'stored_roads': globals.storedRoads,
              'stored_cities': globals.storedCities,
              'game': globals.gameCode
            }),
            headers: headers);
      } else if (globals.putData == 'dice') {
        globals.putData = '';
        response = await http.Client().put(url,
            body: jsonEncode(
                {'dice_num': globals.diceNum, 'game': globals.gameCode}),
            headers: headers);
      } else if (globals.putData == 'theif') {
        globals.putData = '';
        response = await http.Client().put(url,
            body: jsonEncode(
                {'robber_loc': globals.robberLoc, 'game': globals.gameCode}),
            headers: headers);
      } else if (globals.putData == 'next') {
        globals.putData = '';
        response = await http.Client().put(url,
            body: jsonEncode({
        ...
        response = await http.Client().put(url,
            ...
            headers: headers);
      } else if (globals.putData == 'army') {
        globals.putData = '';
        response = await http.Client().put(url,
            body: jsonEncode({
              'largest_army': globals.largestArmy,
              'game': globals.gameCode
            }),
            headers: headers);
      } else if (globals.putData == 'monopoly') {
        print(globals.players['coom']);
        globals.putData = '';
        response = await http.Client().put(url,
            body: jsonEncode(
                {'monopoly': globals.monopoly, 'game': globals.gameCode}),
            headers: headers);
      } else {
        response = await http.Client().put(url,
            body: jsonEncode({'game': globals.gameCode}), headers: headers);
...
class GameBoard extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    globals.xScale = 1 + ((globals.w - 1000) / 1000);
    globals.yScale = 1 + ((globals.h - 1000) / 1000);
    globals.scalar = 1 + ((globals.refscale - 1000) / 1000);
    bgCanvas = canvas;
    ...
    globals.vertices = [];
    for (int i = 0; i < globals.initVertices.length; i++) {
      globals.vertices.add([]);
      globals.vertices[i].add(
          globals.w / 2 + (globals.initVertices[i][0] - 500) * globals.scalar);
      globals.vertices[i].add(
          globals.h / 2 + (globals.initVertices[i][1] - 500) * globals.scalar);
    }

    //scale centers
    globals.tileCenters = [];
    for (int i = 0; i < globals.initCenters.length; i++) {
      globals.tileCenters.add([]);
      globals.tileCenters[i].add(
          globals.w / 2 + (globals.initCenters[i][0] - 500) * globals.scalar);
      globals.tileCenters[i].add(
          globals.h / 2 + (globals.initCenters[i][1] - 500) * globals.scalar);
    }

    //scale xs
    globals.vertexXs = [];
    for (int i = 0; i < globals.initvertexXs.length; i++) {
      globals.vertexXs.add(
          globals.w / 2 + (globals.initvertexXs[i] - 500) * globals.scalar);
    }
...
//draws circles at globals.vertices (used in debugging)
  void drawVertices() {
    ...
    bgCanvas.drawArc(cirRect, 0, 2 * pi, true, paint);
    List adj = globals.vertGraph.getAdj(target);
...
class _PlacePiece extends State<PlaceGamePiece> {
  ...
  Widget build(BuildContext context) {
    if (globals.currPlayer != globals.playerNum)
      globals.placeP = 'h';
    else if (globals.placeP == 'h' && globals.start) globals.placeP = 's';
    if (globals.placeP != '' ||
        (globals.start &&
...
class DrawGamePieces extends CustomPainter {
  ...
  void drawStored() {
    ...
    for (int i = 0; i < globals.storedSettlements.length; i++) {
      Paint paint = Paint()
        ..color = playerColors[globals.storedSettlements[i][1]];
    ...
    for (int i = 0; i < globals.storedRoads.length; i++) {
      Paint paint = Paint()
        ..color = playerColors[globals.storedRoads[i][2]]
        ..style = PaintingStyle.stroke
    ...
    for (int i = 0; i < globals.storedCities.length; i++) {
      Paint paint = Paint()..color = playerColors[globals.storedCities[i][1]];
  ...
  void placeCity() {
    if (globals.players[globals.playerOrder[globals.currPlayer]]['settlements']
        .contains(globals.nearestVert)) {
      //store city
      globals.storedCities.add([
        globals.nearestVert,
        globals.players[globals.playerOrder[globals.playerNum]]['color']
      ]);
      globals.players[globals.playerOrder[globals.currPlayer]]['points']++;
      globals.players[globals.playerOrder[globals.currPlayer]]['settlements']
          .add(globals.nearestVert);
  ...
  void placeRoad() {
    ...
    if (!alreadyOwned &&
      ...
      if (!globals.players[globals.playerOrder[globals.currPlayer]]
        ...
        globals.players[globals.playerOrder[globals.currPlayer]]['road_verts']
            .add(globals.nearestVert);
        globals.roadGraph.addVertex(globals.nearestVert);
      }
      ...
      if (!globals.players[globals.playerOrder[globals.currPlayer]]
        ...
        globals.players[globals.playerOrder[globals.currPlayer]]['road_verts']
            .add(globals.closest);
        globals.roadGraph.addVertex(globals.closest);
      }
      globals.roadGraph.addEdge(globals.nearestVert, globals.closest);
      ...
      if (!globals.start) {
        ...
        var l1 = globals.roadGraph.dfs(v1);
        List l2 = globals.roadGraph.dfs(v2);
  ...
  void roadVis() {
    if (!globals.start) {
      ...
      if (globals.storedNx != nearestX) {
        ...
        globals.possVerts = [];
        for (int i = 0; i < globals.vertices.length; i++) {
          if ((globals.vertices[i][0] / 10).floor() ==
                  (nearestX / 10).floor() &&
  ...
  void settlementVis() {
    ...
    //find nearest x
    for (int i = 0; i < globals.vertexXs.length; i++) {
      // gpCanvas.drawLine(Offset(globals.vertexXs[i], 0),
      //     Offset(globals.vertexXs[i], globals.h), paint);
      if (x < globals.vertexXs[i] + globals.vertexXd / 2) {
        nearestX = globals.vertexXs[i];
        found = true;
        break;
    ...
    if (globals.storedNx != nearestX) {
      ...
      globals.possVerts = [];
      for (int i = 0; i < globals.vertices.length; i++) {
        if ((globals.vertices[i][0] / 10).floor() == (nearestX / 10).floor() &&
            !globals.vertexYs.contains(globals.vertices[i][1].round())) {
          globals.vertexYs.add(globals.vertices[i][1].round());
          globals.possVerts.add(i);
        }
    ...
    if (!found) nearestY = globals.vertexYs[globals.vertexYs.length - 1];
...
class _UIState extends State<UIWidget> {
  ...
  void rollDice() {
    if (!globals.rolled && globals.currPlayer == globals.playerNum) {
      setState(() {
        globals.rolled = true;
        globals.putData = 'dice';
        var rand = Random();
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(children: <Widget>[
      uiButton(context, globals.playerOrder[globals.playerNum], 20, 20, null),
      //dice roll
      Positioned(
          left: 20,
      ...
      if (!globals.rolled &&
          !globals.start &&
          globals.currPlayer == globals.playerNum)
        Align(
            alignment: Alignment(0, -0.9),
            child: (Text('Roll',
                textAlign: TextAlign.center,
                style: TextStyle(fontSize: 40.0, color: colors['white']))))
      else if (!globals.start && globals.rolled)
        Positioned(
            ...
            child: (Text(globals.diceNum.toString(),
                ...
                style: TextStyle(fontSize: 40.0, color: colors['white'])))),
      //players turn at top
      if (globals.currPlayer != globals.playerNum && !globals.start)
        Align(
            alignment: Alignment(0, -0.9),
            child: (Text(globals.playerOrder[globals.currPlayer] + '\'s Turn',
                textAlign: TextAlign.center,
                style: TextStyle(fontSize: 40.0, color: colors['white'])))),
      //trade button
      uiButton(
          ...
          child: SizedBox(
              ...
              child: DecoratedBox(
                  decoration: BoxDecoration(
                      color: colors['beige'],
                      border: Border.all(
                          color: playerColors[globals.players[
                              globals.playerOrder[globals.playerNum]]['color']],
      ...
      Positioned(
        left: globals.w - 20 - globals.refscale * 0.15,
        top: 30 + globals.refscale * 0.67,
        child: Text(
          'VPs: ' +
              (globals.players[globals.playerOrder[globals.playerNum]]['points']
                  .toString()),
      ...
      //largest army card
      if (globals.largestArmy > 2 &&
          globals.largestArmy ==
              globals.players[globals.playerOrder[globals.playerNum]]
                  ['used_dev_cards'][0])
        Positioned(
          left: globals.w - globals.refscale * 0.3 - 20,
      ...
      //resource selection scren
      if (globals.resSelect) ResSelectionScreen(),
      //disp trade screen
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 5; i++) {
      ...
      TextSpan span = new TextSpan(
          ...
          text: globals.players[globals.playerOrder[globals.playerNum]]['hand']
                  [i]
              .toString());
      TextPainter tp = new TextPainter(
          text: span,
        ...
        child: MouseRegion(
            ...
            child: FlatButton(
                ...
                child: SizedBox(
                    ...
                    child: Align(
                        alignment: Alignment(0, -0.7),
                        child: (Text(globals.dispText[card],
                            textAlign: TextAlign.center,
                            style: TextStyle(
                                fontSize: cW * .15,
                                color: colors['brown'])))))))));
...
class _TradeScreen extends State<TradeScreen> {
  ...
  void _addRes(box, i) {
    if (box == 'e') {
      if (globals.players[globals.playerOrder[globals.playerNum]]['hand'][i] !=
          0) {
        equal = true;
        setState(() {
          globals.players[globals.playerOrder[globals.playerNum]]['hand'][i]--;
          expBox.add(i);
        });
      }
    } else {
