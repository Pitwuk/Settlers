...
class ClientMenu extends StatefulWidget {
  @override
  _ClientMenuState createState() {
    return _ClientMenuState();
  }
}

class _ClientMenuState extends State<ClientMenu> {
  void _showLobby() {
    Navigator.of(context).pushNamed('/client/lobby');
  }
    ...
    return Column(children: <Widget>[
      ...
      if (globals.error)
        Padding(
          ...
          child: Text('Game code invalid or name taken',
...
class ClientLobby extends StatefulWidget {
  @override
  _ClientLobby createState() {
    return _ClientLobby();
  }
}

class _ClientLobby extends State<ClientLobby> {
  LobbyManager manager = LobbyManager();

  void _showOrderScreen() {
    Navigator.of(context).pushNamed('/client/order');
  }
...
class LobbyManager {
  ...
  Stream<List<String>> get lobbyNow async* {
    ...
    while (true) {
      ...
      if (json['next_screen'] == '1')
        players = ['%next%'];
      else {
        List players2 = json['player_order'];
        players = List<String>.from(players2);
      }
      yield players;
...
class ClientOrder extends StatefulWidget {
  @override
  _ClientOrder createState() {
    return _ClientOrder();
  }
}

class _ClientOrder extends State<ClientOrder> {
  void _showColorScreen() {
    Navigator.of(context).pushNamed('/client/color');
  }
...
//game board
class ClientGame extends StatefulWidget {
  @override
  _ClientGame createState() {
    return _ClientGame();
  }
}

class _ClientGame extends State<ClientGame> {
  bool init = false;
      ...
      return Scaffold(
        ...
        children: <Widget>[
          ...
          if (!init)
            BlocProvider(
              create: (BuildContext context) =>
                  GameBloc('', '', repository: repository),
              child: BlocBuilder<GameBloc, GameState>(
                builder: (context, state) {
                  if (state is GameEmpty) {
                  ...
                  if (state is GameLoaded) {
                    ...
                    //           init = true;
                    //         }));
                    globals.tiles = state.gamedata.gameData['tiles'];
                    globals.initCenters =
                        state.gamedata.gameData['tile_centers'];
                    globals.initVertices = state.gamedata.gameData['vertices'];
                    globals.robberLoc = state.gamedata.gameData['robber_loc'];
                    globals.initvertexXs = state.gamedata.gameData['vertex_Xs'];
                    globals.coastVerts = state.gamedata.gameData['coast_verts'];
                    globals.dist = state.gamedata.gameData['distribution'];
                    globals.playerOrder =
                        state.gamedata.gameData['player_order'];
                    globals.players = state.gamedata.gameData['players'];
                    globals.numPlayers = state.gamedata.gameData['num_players'];
                    constructGraph();
                    //draw game board
                    WidgetsBinding.instance
                        .addPostFrameCallback((_) => setState(() {
